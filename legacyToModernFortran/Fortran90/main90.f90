program solidification_pure_Ni
  ...
  ! =====
  ! data
  integer,parameter::dp=selected_real_kind(15),sp=selected_int_kind(4) ! Fortran90 feature
  integer(sp),parameter::Nx=100,Ny=100
  integer(sp)::nsteps,istep,i,j,ip,im,jp,jm
  ...
  alambd,b,a0,w,pmob,term1,term2,f,theta,dt,t1,t2,x,y,dist
  ...
  dt      = dx*dx/(5.0*Tk/Cp)
  nsteps  = 6000
  call cpu_time (t1)  ! for serial timing
  !====
  ...
  do istep = 1,nsteps
    ...
    !update fields using whole array    Fortran 90 feature
    T = Tnew
    p = pnew
  end do
  !get the end time
  ...
  !call qplclr (p,Nx,Ny)       ! dislin graphical library for quick plot
